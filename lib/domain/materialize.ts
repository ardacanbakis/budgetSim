...
/**
 * Occurrences in [today, today + monthsAhead] that have no transaction yet
 * (neither planned nor already completed) for their template+date.
 * Both repos run this same algorithm, so materialization is identical
 * in demo and Supabase modes.
 */
export function computeMissingOccurrences(
  ...
  for (const template of templates) {
    for (const dueDate of occurrencesBetween(template, today, horizon)) {
