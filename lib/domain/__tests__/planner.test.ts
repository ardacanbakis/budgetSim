...
describe("planExtraFlows", () => {
  it("emits one flow per hit month, in the item's own currency", () => {
    const flows = planExtraFlows(
      { items: [item({ amount: 900, currency: "TRY", inflates: false, durationMonths: 2 })], budgets: [], devaluation: off, lostIncome: [], loans: [], funding: NO_FUNDING },
      6,
      FIRST
    );
    ...
    const flows = planExtraFlows(
      {
        ...
        devaluation: deval30, lostIncome: [], loans: [], funding: NO_FUNDING,
      },
      13,
      FIRST
    );
  ...
  it("skips disabled and zero entries, and reports replaced categories", () => {
    const plan = {
      ...
      devaluation: off, lostIncome: [], loans: [], funding: NO_FUNDING,
    };
    expect(planExtraFlows(plan, 3, FIRST)).toHaveLength(3); // only budget "a"
    expect([...planReplacedCategories(plan)]).toEqual(["a"]);
    expect(planIsEmpty({ items: [], budgets: [], devaluation: off, lostIncome: [], loans: [], funding: NO_FUNDING })).toBe(true);
    expect(planIsEmpty({ items: [], budgets: [], devaluation: deval30, lostIncome: [], loans: [], funding: NO_FUNDING })).toBe(false);
  });
});
